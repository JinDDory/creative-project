...
public class MemberController {
    ...
    public int handler(Header recvPt) throws IOException {
...
//        Header sendPt = new Header(Protocol.TYPE_RESPONSE, Protocol.T2_CODE_SUCCESS);
//        sendPt.send(out);
//        return USER_UNDEFINED;
//    }

    private int requestCreate(Header header) {
        switch (header.getEntity()) {
            case Protocol.ENTITY_FAVORITES:   // 즐겨찾기 생성
        }
        return 0;
    }

    private int requestRead(Header header) {
        switch (header.getEntity()) {
